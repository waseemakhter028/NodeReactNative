...
const getDeliveryCharge = (req, res) => {
  try {
    ...
    return helper.sendSuccess(delivery_charge, res, req.t("data_retrived"), 200)
...
const getCheckoutInfo = async (req, res) => {
  try {
    ...
    data['cart'] = await Cart.aggregate([
      ...
      {
        $project: { id: "$_id", _id: 0, image: "$product.image", name: "$product.name", price: "$product.price", user_id: 1, product_id: 1, quantity: 1 }
      }
    ])
    data['address'] = await Address.find({ user_id: helper.ObjectId(user_id) }).sort({ createdAt: -1 })

    let coupons = await UserCoupon.find({ user_id: helper.ObjectId(user_id), used: 0 }).populate({ path: 'coupon_id', select: '_id title code description validity' })
...
const checkOrder = async (req, res) => {
  try {
    ...
    const cart = await Cart.find({ user_id: helper.ObjectId(user_id) }, { id: 1, user_id: 1, quantity: 1 })
      .populate({ path: 'product_id', select: '_id qty name price status' }).sort({ createdAt: -1 })
    ...
    for (let row of cart) {
      if (row.product_id.status !== 1)
        return helper.sendError({}, res, row.product_id.name + '' + req.t("order_product_status"), 200)
      else if (row.quantity > 20)
        return helper.sendError({}, res, row.product_id.name + '' + req.t("order_qty_limit"), 200)
      else if (row.product_id.qty === 0)
        return helper.sendError({}, res, row.product_id.name + '' + req.t("order_out_of_stock"), 200)
      else if (row.quantity > row.product_id.qty)
        return helper.sendError({}, res, row.product_id.name + '' + req.t("order_available_qty") + '' + row.product_id.qty, 200)
...
const checkCoupon = async (req, res) => {
  ...
  const couponInfo = await Coupon.findOne({ 'code': coupon_code }, { _id: 1, validity: 1, status: 1, code: 1 })

  if (!couponInfo)
    return req.t('coupon_invalid')

  const userCouponInfo = UserCoupon.findOne({ user_id: helper.ObjectId(user_id), coupon_id: helper.ObjectId(couponInfo._id) })
  ...
  if (couponInfo.status != 1) {
    ...
    let curDate = moment.utc().tz(req.headers['timezone']).format('YYYY-MM-DD')
    let couponDate = moment.utc(couponInfo.validity).tz(req.headers['timezone']).format('YYYY-MM-DD')
    if (couponDate < curDate)
      return req.t('coupon_expired')
  }
...
const insertCouponLog = (data) => {
  ...
  UserCoupon.findByIdAndUpdate(id, {
    used: 1
  }, { new: true })
    .then(couponUsed => {
...
const insertOrder = async (data) => {
  const { user_id, transaction_id, transaction_detail, payment_method, notes, address_id, delivery_charge } = data
...
const couponCheckout = async (req, res) => {
  ...
  try {
    const userCart = await Cart.find({ user_id: helper.ObjectId(req.body.user_id) }, { id: 1, user_id: 1, quantity: 1 })
      .populate({ path: 'product_id', select: '_id qty name price status' }).sort({ createdAt: -1 })

    let subTotal = 0

    userCart.map((row, inx) =>
      subTotal += +row.product_id.price * +row.quantity
    )
    ...
    if (!_.isEmpty(req.body.coupon_code)) {
      ...
      if (check == 1) {
        couponInfo = await Coupon.findOne({ 'code': req.body.coupon_code }, { _id: 1, validity: 1, status: 1, code: 1 })

        userCouponInfo = await UserCoupon.findOne({ user_id: helper.ObjectId(req.body.user_id), coupon_id: helper.ObjectId(couponInfo._id) })
      }
      else {
        return helper.sendError({}, res, check, 200)
      }
...
const placeOrder = async (req, res) => {
  ...
  const { user_id, month, payment_method, year } = req.body


  if (payment_method === "stripe") {
    const cmonth = parseInt(moment().format('M'))
    const cyear = parseInt(moment().year())

    let cardMonth = month.toString()

    cardMonth = parseInt(cardMonth.replace(/^0+/, ''))

    if (year == cyear && cardMonth < cmonth) {
      return helper.sendError({}, res, req.t("invalid_card_month"), 200)
    }
  ...
  try {
    ...
    if (req.body.payment_method == 'stripe')
      order = await stripeCheckout(req, res)
    ...
    helper.sendNotification(user_id, req.t('order_confirmed'), `${req.t('your_order')} ${order.order_id} ${req.t('is_now')} ${status}`)
...
const orders = async (req, res) => {
  try {
    ...
    const rows = await Order.aggregate([
      ...
      { $project: { id: "$_id", _id: 0, order_id: 1, rated: 1, notes: 1, status: 1, total: 1} },
...

      {
        $facet: {
          ...
          metaData: [
            ...
            { $project: { id: 1,  order_id: 1, rated: 1, notes: 1, status: 1, total: 1  } }
          ]
        }
      }
    ])
