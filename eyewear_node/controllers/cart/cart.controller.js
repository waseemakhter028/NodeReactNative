...
const getCart = async (req, res) => {
  try {
       ...
       if(!user) 
        ...
        const data =  await Cart.find({ user_id: helper.ObjectId(user_id)},{id:1,user_id:1,quantity:1})
         .populate({path:'product_id',select: '_id image name price'}).sort({createdAt:-1})
...
const addCart = async (req, res) => {
    try {
      let { user_id, quantity, product_id} = req.body
