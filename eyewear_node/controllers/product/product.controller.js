...
const getLatestProducts = async (req, res) => {
  try {
    const rows = await Product.find(
      {},
      { id: "$_id", _id: 0, name: 1, price: 1, image: 1 }
    ).limit(4);
...
const showProduct = async (req, res) => {
  try {
    ...
    const product = await Product.findById(product_id).select(
      "_id name image price qty category information"
    );
    let data = [];

    if (!product) return helper.sendError({}, res, "product not found", 405);
    ...
    let relatedProducts = await Product.aggregate([
      {
        $match: {
          status: { $eq: 1 },
          category: { $eq: helper.ObjectId(product.category) },
          _id: { $ne: helper.ObjectId(product.id) },
        },
      },
      {
        $sample: { size: 4 },
    ...
    data.push({
      extra: {
        ...
        reviewsCount: !_.isEmpty(productRating[0].reviewCount)
          ? productRating[0].reviewCount[0].reviewCount
          : 0,
        avg_rating: !_.isEmpty(productRating[0].avgRating)
          ? productRating[0].avgRating[0].avgRating
          : 0,
        isCart: isCart,
      },
      product: product,
      reviews: !_.isEmpty(productRating[0].comments)
        ? productRating[0].comments
        : [],
